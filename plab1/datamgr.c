...
// Helper function: free an element
static void element_free(void **element) {
    free(*element);
    *element = NULL;
}

// Helper function: find sensor node by ID
...
// Helper function: update the running average
...
// Return the room ID associated with a sensor
uint16_t datamgr_get_room_id(sensor_id_t sensor_id) {
    element_t *data = get_sensor_node_by_id(sensor_id);
    return (data != NULL) ? data->room_id : 0;
}

// Return the running average temperature of a sensor
sensor_value_t datamgr_get_avg(sensor_id_t sensor_id) {
    element_t *data = get_sensor_node_by_id(sensor_id);
    return (data != NULL) ? data->running_avg : 0;
}

// Return the last modified timestamp of a sensor
time_t datamgr_get_last_modified(sensor_id_t sensor_id) {
    element_t *data = get_sensor_node_by_id(sensor_id);
    return (data != NULL) ? data->last_modified : 0;
}
